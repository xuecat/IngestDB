    ...
    public partial class TaskManager
    {
        ...
        public async Task UpdateTaskMetaDataAsync(int taskid, MetaDataType type, string metadata)
        {
            ...
            if (item != null)
            {
                ...
                await Store.UpdateTaskMetaDataAsync(item, true, o => o.Metadatalong);
            }
        }
            ...
            if (metadata != null && metadata.Length > 0)
            {
                ...
                if (taskMetadata != null)
                {
                    ...
                    if (!string.IsNullOrEmpty(dbmetadata) && !string.IsNullOrEmpty(metadata))
                    {
                        ...
                        await Store.UpdateTaskMetaDataAsync(taskMetadata, true, o => o.Metadatalong);
                        return dbmetadata;
                    }
                }
        ...
        public async virtual ValueTask<string> UpdateMetadataPropertyAsync(int taskid, MetaDataType type, List<PropertyResponse> lst)
        {
            ...
            try
            {
                ...
                if (material != null)
                {
                    ...
                    taskMetadata.Metadatalong = root.ToString();
                    await Store.UpdateTaskMetaDataAsync(taskMetadata, true, o => o.Metadatalong);
                    return taskMetadata.Metadatalong;
                }
            }
            catch (Exception e)
            {
        ...
        public async ValueTask<int> TrimTaskBeginTime(int taskid, string StartTime)
        {
            ...
            if (taskinfo.State != (int)taskState.tsComplete && (taskinfo.Tasktype != (int)TaskType.TT_PERIODIC || (taskinfo.Tasktype == (int)TaskType.TT_PERIODIC && taskinfo.OldChannelid > 0)))
            {
                ...
                else if (taskinfo.Tasktype == (int)TaskType.TT_VTRUPLOAD)
                {
                    ...
                    if (await Store.AdjustVtrUploadTasksByChannelId(taskinfo, dtNow, false))
                    {
        ...
        public async Task<TaskContentResponse> ModifyTaskDB(TaskContentRequest task)
        {
            ...
            var dbptask = _mapper.Map<DbpTask>(task);
