...
namespace IngestTaskPlugin.Managers
{
    ...
    public partial class TaskManager
    {
        ...
        {
            int QueryUseHotTable = UseHotTableConvert.QueryUseHotTable(oldtaskid);
            var item = await Store.GetTaskNotrackAsync(a => a.Where(b => b.Taskid == QueryUseHotTable));
            if (item != null)
            {
                ...
                var lstmeta = await Store.GetTaskMetaDataListNotrackAsync(a => a.Where(b => b.Taskid == oldtaskid));
            ...
            if (item != null && item.Signalid > 0)
            {
                ...
                if (matchlst != null && matchlst.Count > 0)
                {
                    ...
                    if (freeChannelIdList != null && freeChannelIdList.Count > 0)
                    {
                        ...
                        await Store.UpdateTaskAsync(item, "", true, o => o.Channelid, o => o.SyncState, o => o.DispatchState);
                        return item;
                    }
        ...
        public async Task<List<TResult>> GetNeedSynTasks<TResult>()
        {
            ...
            return _mapper.Map<List<TResult>>(await Store.GetTaskListNotrackAsync(x => x.Where(y => y.State == QueryUseHotTable
                                                                                             && DateTime.Now < y.Endtime
                                                                                             && y.OpType != (int)opType.otDel)));
        }
