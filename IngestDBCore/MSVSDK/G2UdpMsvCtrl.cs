...
namespace IngestDBCore.MSVSDK
{
    public class G2UdpMsvCtrl
    {
        ...
        public string GetMsvUdpData(string strlocip, int nlocport, string strmsvip, int nmsvchport, string strcmd, int nTimeOut = 4)
        {
            string strret = "";
            ...
            MsvClientCtrlSDK.Logger.Info(  strtmplog +", strcmd:" + strcmd);
            ...
            try
            {
                ...
                while (true)
                {
                    ...
                    if (ts.TotalSeconds > nTimeOut)
                    {
                        ...
                        MsvClientCtrlSDK.Logger.Info(  strtmplog + ",Recv Time out" );
                        break;
                    }
                ...
                if (nRecvedLen > 0)
                {
                    ...
                    strtmplog = string.Format("{0}:{1}", strmsvip, nmsvchport);
                    MsvClientCtrlSDK.Logger.Info(  strtmplog  + ",Recv:" +strret);
                }
                else
                {
                    MsvClientCtrlSDK.Logger.Info("Recv: len error");
                }
                udpClient.Close();
                udpClient = null;
                bytedata = null;
            ...
            catch (System.ObjectDisposedException ex)
            {
                udpClient.Close();
                udpClient = null;
                MsvClientCtrlSDK.Logger.Info(  strtmplog + "GetMsvUdpData Error!errorinfo=" + ex.ToString());
            }
